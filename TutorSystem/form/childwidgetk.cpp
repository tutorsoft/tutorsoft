...
{
    ui->setupUi(this);
    MainWidget::allSignalEvent.setChildWidgetK(this);
    selectrow = 0;
    QStringList header;
...
void ChildWidgetK::showEvent(QShowEvent *event)
{
    ...
    QString strsend =  "ChildWidgetK_queryAll:" + tstr0;
    emit sendWriteSocket(strsend);
}

void ChildWidgetK::onHeaderClicked(int row,int column)
{
    selectrow = row;
}

void ChildWidgetK::acceptSocketData(QString ret)
{
    qDebug()<<"ChildWidgetK==>"<<ret;
