...
void TcpSocket::readData()
{
    ...
    if(t_flog == "login"){
        ...
        if(ret == false){
            ...
            this->write(retstr.toStdString().c_str());
            //教师评价
            bool rets = query.exec(QString("create table %1(evaluateid varchar(20) primary key,evaluate_student varchar(20),evaluate_text varchar(100))").arg(QString("evaluate_%1").arg(t_strlist.at(2))));
            rets = query.exec(QString("insert into %1 values('0001','张三','666')").arg(QString("evaluate_%1").arg(t_strlist.at(2))));
        ...
        if(ret == false){
            qDebug()<<"query"<<query.lastError();
            QString retstr = "EvaluateWidget:false";
            this->write(retstr.toStdString().c_str());
        }else{
            QString retstr = "EvaluateWidget:true";
            this->write(retstr.toStdString().c_str());
        }
        ...
        if(ret == true){
            ...
            QString retstrs;
            qDebug()<<"retvector.size()=="<<retvector.size();
            retstrs.append("EvaluateWidget_queryAll:");
            retstrs.append(QString::number(retvector.size()));
            retstrs.append(":");
            retstrs.append(retstr);
            qDebug()<<"asdfasdf"<<retstrs;
